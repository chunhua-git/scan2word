...
把法院 / 法拍的扫描件 PDF 拖进界面，输出可以直接改字、另存 PDF 交付的 `.docx`。

## 适用范围

**核心管线不挑文档类型**：PDF 解析、表格网格重建、勾选框 / 印章 / 空白栏还原、
Word 写出，这些跟「是不是法院文书」没关系——有框线的表格就是有框线的表格。

已实测的文档类型：

| 类型 | 结果 |
|---|---|
| 法院文书（送达地址确认书、领款人收款账号确认书等，扫描件与电子版） | 逐字比对 0.00%，表格 / 合并单元格 / 勾选框 / 空白栏全部还原 |
...
- `core/lexicon.py` 的形近字纠正：**只在"替换后能命中词表里的词"时才生效**。
  非法律文档下它等于不工作，但也不会把对的字改错。
  词表可扩展，见 `core/data/`（`法律词表.txt` 内置 + `自定义词表.txt` 给你加自己领域的词）。
- `core/verify.py` 的数字格式校验（案号 / 身份证 / 手机号）：非对应文档下只是少校验几项。
- 法院名校验：只对以「人民法院」结尾的行生效。

**尚未实测**的类型（设计上通用，但没跑过真实样本）：无框线表格（靠空白分栏）、
多栏排版（论文 / 报纸）、竖排文字、手写体。有样本的话欢迎提 issue。
...
core/                   与界面完全解耦的核心
  ...
  verify.py             逐字比对、数字格式校验（身份证校验位 / Luhn / 案号）
...
`scan2word` 文件夹拷到目标机器，双击 `scan2word.exe`。
不需要 Python、不需要联网、不需要独立显卡。
